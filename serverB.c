...
int main(int argc, char *argv[]){  
    ...
    addr.sin_port=htons(serverB_port);  
    addr.sin_addr.s_addr=htonl(INADDR_ANY) ;  
    if (bind(sockfd, (struct sockaddr *)&addr, sizeof(addr))<0){  
        perror("connect");  
        exit(1);  
    } 
    printf("The server B is up and running using UDP on port 22422 \n");
    while(1){  
        ...
        FILE *fp = fopen("backendB.txt","r");
        if((fp = fopen("backendB.txt","r")) == NULL) //
        { 
        printf("error!"); 
         return -1; 
        } 
